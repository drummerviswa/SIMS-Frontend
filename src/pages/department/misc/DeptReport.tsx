...
export default function DeptReport() {
  ...
  const [loggedInDepartment, setLoggedInDepartment] =
    useState<StoredDepartment | null>(null);
  const [activeTab, setActiveTab] = useState<string>("overview"); // 'overview', 'students', 'faculty'

  // Overview states
  const [departmentOverviewData, setDepartmentOverviewData] =
    useState<SimplifiedDepartmentData | null>(null);
    ...
    if (storedDept) {
      try {
        ...
        if (parsedDept && parsedDept.deptid && parsedDept.deptName) {
          ...
          fetchData<SimplifiedDepartmentData>(
            `/reports/department?id=${parsedDept.deptid}`,
            setDepartmentOverviewData
          );
        } else {
          setError(
            "Invalid department data in your login session. Please re-login."
          );
      ...
      setError(
        "No department found in your login session. Please ensure you are logged in correctly."
      );
      setLoggedInDepartment(null);
      setLoading(false);
    }
  ...
  const fetchStudentByRegNo = useCallback(async () => {
    ...
    try {
      ...
      if (res.data.student.departmentName !== loggedInDepartment.deptName) {
        setStudentError(
          `Student ${studentReg} does not belong to ${loggedInDepartment.deptName}.`
        );
        setStudentData(null);
      } else {
  ...
  const fetchFacultyList = useCallback(async () => {
    if (!loggedInDepartment?.deptid) {
      setFacultyListError(
        "Department not identified. Cannot fetch faculty list."
      );
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      const res = await API.get<Faculty[]>(
        `/admin/manage/faculty/department/${loggedInDepartment.deptid}`
      );
  ...
  useEffect(() => {
    if (selectedFacultyId && !facultyData) {
      // Only fetch if ID set and data not loaded
  ...
  return (
    <div className="p-6 max-w-screen-xl mx-auto dark:bg-gray-900 dark:text-white min-h-screen">
      <h1 className="text-3xl font-bold mb-6 text-gray-800 dark:text-white">
        {loggedInDepartment?.deptName
          ? `Department Dashboard: ${loggedInDepartment.deptName}`
          ...
          <div id="department-overview-content">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              <StatCard
                ...
                value={departmentOverviewData.totalStudents}
              />
              <StatCard
                title="Total Faculty"
                value={departmentOverviewData.totalFaculty}
              />
              <StatCard
                title="Total Courses"
                value={departmentOverviewData.totalCourses}
              />
              {departmentOverviewData.averageDepartmentCGPA && (
                <StatCard
                  title="Average CGPA (Proxy)"
                  value={departmentOverviewData.averageDepartmentCGPA}
                />
              )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <ChartContainer title="Department-wide Grade Distribution">
                <PieChart>
                  ...
                  >
                    {(departmentOverviewData.gradeDistribution || []).map(
                      (_, index) => (
                        <Cell
                          key={`cell-${index}`}
                  ...
                  <Tooltip
                    formatter={(value) => [value, "Students/Subjects"]}
                  />
                  <Legend />
                </PieChart>
              </ChartContainer>
            ...
            <ExportButtons
              ...
              filename={`department_overview_${departmentOverviewData.department.deptName.replace(
                /\s+/g,
                "_"
              )}`}
      ...
      {activeTab === "overview" &&
        ...
        !error && (
          <p className="text-gray-600 dark:text-gray-400 mt-4">
            No overview data available for your department.
          </p>
        )}
      ...
      {activeTab === "students" && loggedInDepartment && (
        <div className="py-4">
          <h2 className="text-2xl font-bold mb-4 text-gray-800 dark:text-white">
            Search Student in {loggedInDepartment.deptName}
          </h2>
          <div className="mb-6">
            ...
            {studentError && (
              <p className="text-red-600 mt-2">{studentError}</p>
            )}
          </div>
          ...
          {studentData && (
            <div id="student-report-content">
              ...
              {/* ID for PDF export */}
              <h3 className="text-xl font-semibold mb-4 text-gray-800 dark:text-gray-200">
                Report for {studentData.student.sName} (
                {studentData.student.regNo})
              </h3>
      ...
      {activeTab === "faculty" && loggedInDepartment && (
        <div className="py-4">
          <h2 className="text-2xl font-bold mb-4 text-gray-800 dark:text-white">
            Faculty in {loggedInDepartment.deptName}
          </h2>

          {!selectedFacultyId && ( // Show list of faculty if none is selected
            <>
              {facultyListError && (
                <p className="text-red-600 mb-4">{facultyListError}</p>
              )}
          ...
          {selectedFacultyId &&
            facultyData && ( // Show individual faculty report
              <div id="faculty-report-content">
                {" "}
                {/* ID for PDF export */}
                <button
                  onClick={() => {
      ...
      {!loggedInDepartment && !loading && (
        <p className="text-red-600 dark:text-red-400 mt-8 text-center text-lg">
          No department information found. Please ensure you are logged in as a
          department user.
        </p>
      )}
    </div>
