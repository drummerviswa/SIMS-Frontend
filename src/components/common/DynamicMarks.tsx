...
const DynamicMarkTable: React.FC<DynamicMarkTableProps> = ({
  ...
  const [isChangesMade, setIsChangesMade] = useState(false); // Track if changes were made
  ...
  const handleUpdate = async () => {
    if (window.confirm("Do you want to update the marks?")) {
      setIsTableLocked(false); // Unlock table after confirmation
    }
  };
      ...
      data[0].SubCriteriaBreakdown.map((sub) => ({
        ...
        cell: ({ row, column, table }) => {
          ...
          const handleBlur = () => {
            ...
            setIsChangesMade(true); // Mark that changes have been made
            rerender();
          };
